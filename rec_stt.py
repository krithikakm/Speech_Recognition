...
class GSpeech(object):
        ...
        if msg.data:
            if not self.started:
                ...
                if not self.recognising:
                    self.do_recognition()
		print('gspeech recogniser started')
    ...
    def stop_callback(self, msg):
        ...
        if msg.data:
            if self.started:
                self.started = False
	    print('gspeech recogniser stopped')
	...
	print('recording started')
        for i in range(0, int(self.pa_rate / self.pa_buf_len * 3)):
